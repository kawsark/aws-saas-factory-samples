# SaaS tenant isolation example for Amazon Bedrock with ABAC using AWS STS support for tags in JWT
...
1. Application calls AWS STS `AssumeRoleWithWebIdentity` API with the JWT for the 
   signed-in user
2. Application uses the scoped temporary security credentials to access Amazon Bedrock KnowledgeBase resources
...
## Create an Amazon Bedrock Knowledge Base resource

You can create an Amazon Bedrock Knowledge Base using the [AWS Management Console](https://docs.aws.amazon.com/bedrock/latest/userguide/knowledge-base-create.html) or the [aws bedrock create-knowledge-base](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/bedrock-agent/create-knowledge-base.html) CLI command. 

Alternatively, you can use AWS CDK to create it using an Infrastructure-as-Code approach. This [blog post](https://aws.amazon.com/blogs/machine-learning/build-an-end-to-end-rag-solution-using-knowledge-bases-for-amazon-bedrock-and-the-aws-cdk/) and the accompanying repo provides an end to end example for creating a Bedrock Knowledge Base.

Once you create the Knowledge Base, run the following command using the AWS CLI as an Administrator. Adjust the `REGION` for the AWS Region where the Knowledge Base resource exists. Also, adjust `KNOWLEDGE_BASE_ID` for the Knowledge Base ID you created. 

Note that the Knowledge Base ID is different than the Knowledge Base Name. You can get the Knowledge Base ID from the AWS Console by navigating to the Amazon Bedrock, Knowledge Bases, and choosing the specific Knowldge Base from the list.
...
The remainder of this README will guide how to use ABAC to test access to this Knowledge Base.
...
cd aws-saas-factory-samples/saas-tenant-isolation-abac-jwt-sts-bedrock
```
...
Amazon Bedrock Knowledge Base acts as the tenant data store. 
